...
wxBitmap* opencpn_plugin::GetPlugInBitmap() {
  if (!m_default_icon) m_default_icon = new wxBitmap(32,32);
  return m_default_icon;
}

void opencpn_plugin::SetDefaultIcon(wxBitmap* icon) {
  m_default_icon = icon;
}

wxBitmap* opencpn_plugin::m_default_icon;

static void SetDefaultIcon(wxBitmap* bitmap) { opencpn_plugin::m_default_icon = bitmap; }
